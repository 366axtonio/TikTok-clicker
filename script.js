...
// === Admin Auto-Login für Firebase ===
firebase.initializeApp(firebaseConfig);
const auth = firebase.auth();
const db = firebase.database();

const ADMIN_EMAIL = "admin@clicker.com";
const ADMIN_PASSWORD = "366axtonio";
  ...
  .catch((error) => {
    if (error.code === "auth/user-not-found") {
      auth.createUserWithEmailAndPassword(ADMIN_EMAIL, ADMIN_PASSWORD)
        .then(() => console.log("Admin-Account erstellt"))
        .catch(err => console.error("Admin-Erstellung fehlgeschlagen", err));
    } else {
